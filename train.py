...
```
python3 train.py --data path/to/FAT --object soup --outf soup --gpuids 0 1 2 3 4 5 6 7 
python3 scripts/train.py --data /Users/pocoder/Downloads/fat --datatest '' --object soup --outf soup

python3 scripts/train.py --pretrained false --datatest /Users/pocoder/Downloads/2022_05_28/rendered_dataset/test --object container --net /Users/pocoder/Downloads/2022_05_28/Deep_Object_Pose/train_container_05_29_2022/net_container_60.pth
```
...
def load_json(path, objectsofinterest, img):
    """
...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        beliefs = create_belief_map(
            img, 
            points_belief=keypoints,
            nbpoints = 9,
            sigma = self.sigma)

        beliefs = transforms.Resize((img.shape[2] // 8, img.shape[1] // 8))(beliefs)
        #print("beliefs map shape: ", beliefs[0].shape)

        affinities = generate_map_affinity(img, 8, keypoints, centroids, 8)
...
def generate_map_affinity(img, nb_vertex, points_interest, objects_centroid, scale):
    """
    ...
    Args:
        ...
        points_interest: list of points
    ...
    for i_pointsImage in range(len(points_interest)):
        pointsImage = points_interest[i_pointsImage]
        center = objects_centroid[i_pointsImage]
        for i_points in range(nb_vertex):
            point = pointsImage[i_points]
            affinity_pair, img_affinity = get_affinity_center(int(img.shape[2] / scale),
...
def get_affinity_center(width, height, point, center, radius=7, img_affinity=None):
    """
    Function to create the affinity maps, 
    e.g., vector maps pointing toward the object center. 
...
def create_belief_map(img, points_belief, nbpoints, sigma=16):
    """
    Args: 
        img: image
        points_belief: list of points in the form of
...
def _create_belief_map(img, points_belief, nbpoints, sigma=16):
    """
    Args: 
        img: image
        points_belief: list of points in the form of
    ...
    for numb_point in range(nbpoints): 
        ...
        for point in points_belief:
            ...
            w = int(sigma*2)
                ...
                #    for y in range(int(p[1])-w, int(p[1])+w):
                #        array[y,x] = np.exp(-(((x - p[0])**2 + (y - p[1])**2)/(2*(sigma**2))))
            if p[0]-w>=0 and p[0]+w<img.shape[1] and p[1]-w>=0 and p[1]+w<img.shape[2]:
                for y in range(int(p[0])-w, int(p[0])+w):
                    for x in range(int(p[1])-w, int(p[1])+w):
                        array[y,x] = np.exp(-(((y - p[0])**2 + (x - p[1])**2)/(2*(sigma**2))))

        # Well, lets transpose it for consistency with the other code
        array = np.transpose(array)
        beliefsImg.append(array)
    return torch.from_numpy(np.array(beliefsImg))


def make_grid(tensor, nrow=8, padding=2, normalize=False, img_range=None, scale_each=False, pad_value=0):
    """
...
def main():
    ...
    if opt.data != '':
        ...
        # print(training_data)
        # for i in range(2):
        #     images = iter(training_data)
        #     im = next(images)
        #     save_image(im['image'], '{}/train_{}.png'.format(opt.outf, str(i).zfill(5)), mean=mean[0], std=std[0])
    ...
    if testing_data is not None:
        print("testing data: {} batches".format(len(testing_data)))
    ...
    if opt.net != '':
        net.load_state_dict(torch.load(opt.net, map_location=torch.device(device)))
    ...
    def run_network(epoch, loader, train=True, scaler=None, pbar=None):
        nonlocal nb_update_network
        # net
        if train:
            net.train()
        else:
        ...
        for batch_idx, targets in enumerate(loader):
            ...
            use_cuda = torch.cuda.is_available()
            device = torch.device("cuda" if use_cuda else "cpu")
            data = Variable(targets['image'].to(device))
            ...
            with amp.autocast():
                ...
                for l in output_belief: #output, each belief map layers.
                    if loss is None:
                        #print("l.shape:", l.shape, "target_belief.shape:", target_belief.shape)
    ...
    for epoch in pbar:
        ...
        if training_data is not None:
            run_network(epoch, training_data, scaler=scaler, pbar=pbar)

        if opt.datatest != "":
            run_network(epoch, testing_data, train=False, pbar=pbar)
            if opt.data == "":
